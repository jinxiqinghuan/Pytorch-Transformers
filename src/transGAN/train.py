...
parser.add_argument('--gener_batch_size', type=int, default=4 , help='Batch size for generator.')
parser.add_argument('--dis_batch_size', type=int, default=4, help='Batch size for discriminator.')
...
def train(noise,generator, discriminator, optim_gen, optim_dis,
    ...
    for index, (img, _) in enumerate(train_loader):
        ...
        if gen_step and index % 100 == 0:
            ...
            save_image(sample_imgs.detach(), f'generated_images/generated_img_{epoch}_{index % len(train_loader)}.jpg', nrow=2, normalize=True, scale_each=True)            
